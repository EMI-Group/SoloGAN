...
class _CBINorm(_BatchNorm):  # num_con means conditional code length, while num_features means number of input channel
    def __init__(self, num_features, num_con=8, eps=1e-5, momentum=0.1, affine=False):
        super(_CBINorm, self).__init__(num_features, eps, momentum, affine)
        self.ConBias = nn.Sequential(nn.Linear(num_con, num_features), nn.Tanh())    ## each layer has a separate condition embedding
    ...
    def forward(self, input, ConInfor):
        ...
        tarBias = self.ConBias(ConInfor).view(b, c, 1, 1)    # The condition is linearly projected to each layer's gain and biases  
        ...
        if self.affine:    # where gamma and beta can be self-learned 
